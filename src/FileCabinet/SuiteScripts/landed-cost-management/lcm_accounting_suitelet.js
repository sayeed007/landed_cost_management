...
define(['N/log', 'N/ui/serverWidget', 'N/url', './lcm_po_selection_config', './lcm_accounting_lib'], (
  ...
  function renderBillLines(groups) {
    ...
    const mismatches = (groups || [])
      ...
      .map(
        (line) =>
          `<li>Looking for <code>${escapeHtml(line.expectedSourceKey)}</code><br>Found on the Bill: ${
            (line.markersOnBill || []).length
              ? (line.markersOnBill || []).map((key) => `<code>${escapeHtml(key)}</code>`).join('<br>')
              : '<em>no LCM Source Key on any line</em>'
          }</li>`
      )
      .join('');
    ...
    return `<h4>Vendor Bill lines to be written</h4><table class="lcm-table"><thead><tr><th>From LCM Rows</th><th>LC Cost Category</th><th>LC Cost Item</th><th>Allocation Method</th><th>Qty</th><th>Amount</th><th>Action</th></tr></thead><tbody>${rows}</tbody></table>${
      mismatches
        ? `<p class="lcm-muted">A line merges into an existing one only when that line carries the same LCM Source Key. A Vendor Bill generated by an earlier build carries a key in the older format, so its lines will not be merged into - recreate the Bill rather than appending to it.</p><ul>${mismatches}</ul>`
        : ''
    }`;
  }
