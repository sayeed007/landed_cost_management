...
- `Repair Vendor Bill Lines` is the only path that edits an already-created Vendor Bill, and it is explicit: it is a button, it previews what it would change, and it requires confirmation. It consolidates duplicate generated cost lines that belong to this LCM record into one line per merge identity, sets quantity `1`, merges the distinct descriptions, and re-tags a generated line whose Cost Category is missing. It is total-preserving - the surviving line carries the sum of the lines it replaces, never the Landed Cost row total - so a Vendor Bill total can never move. It does not touch any other line on the Bill, does not change the Landed Cost rows, and does not change allocation. Use it for Vendor Bills generated before source-row merging worked; no automatic flow calls it.
