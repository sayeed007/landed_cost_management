...
- The client and child User Event source matching available defaults: Subsidiary, Document Type, Currency, Exchange Rate, Effective Date, fixed hidden Bill Type, fixed hidden Bill Line Type, Allocation Method, mapped Cost Category, and LC Cost Item. LCM Items instead carry a read-only Receiving Location from the source PO line, with the PO header location as fallback. The PO selector writes the Location ID and display text before first save, while a missing persisted value is backfilled on LCM form load. Expense Account is retired because Vendor Bills always use item lines; Journal account fallback remains script-configured. Each field is applied independently so one unavailable or invalid default does not block the remaining defaults.
...
- `Create Bill` processes uncreated Landed Cost rows marked `Bill` through explicit routing. `Append to Existing Bill` defaults clear: blank `Bill Group` creates a separate new Vendor Bill for each row, while equal nonblank Bill Group labels create one new Bill. Checking Append requires an explicit Target Vendor Bill, so no code silently chooses between compatible Bills such as `#123` and `#125`.
- An append target must be an editable Vendor Bill already created by the same LCM and match Vendor, Subsidiary, Currency, and effective Allocation Method. Different LC Cost Categories are allowed on the target Bill, so an Amendment Charge can be routed with an Opening Charge. A Bill Group or target with conflicting methods is rejected because Cost Allocation Method is a Bill body field. Exchange rate remains source-row authoritative for base-currency allocation; the Bill header retains its first rate.
- Before a Vendor Bill is saved, LCM writes and reads back its native Cost Allocation Method. It must equal the effective method from the routed LCM rows; the transaction is not saved when NetSuite retains a form default instead. When appending to a Bill made by this LCM whose every marked cost line uses the requested method, LCM may repair a stale defaulted header (for example, `Weight` to `Value`). Bills containing manual or foreign lines remain protected by the normal strict method mismatch validation.
- Within each deliberately chosen Vendor Bill, compatible Landed Cost rows merge into one Vendor Bill item line when Vendor, Subsidiary, Currency, LC Cost Category, LC Cost Item, and Allocation Method all match. Effective Date, Department, and Class do not split the merge group; if they differ, the generated Bill line uses the first source row's values. The original Landed Cost rows remain separate and are all linked back to the created Bill. Already-created Vendor Bills are not rewritten by `Recalculate Landed Cost`.
...
- `Cost Allocation Method` is a **body** field, so one explicit append target or pending Bill Group has exactly one of them. Landed Cost rows routed to the same Bill that disagree about the Allocation Method are refused and the preview names both methods and the affected rows. Use a separate Bill Group or separate Bill target for the differing rows.
...
- Journal Entry grouping remains by subsidiary and currency. Vendor Bill rows never implicitly append to an earlier compatible Bill; only an explicit Target Vendor Bill permits an append.
...
- Before confirmation, the preview checks the transformed receipt for lines where NetSuite requires Inventory Detail, reads the assignment fields NetSuite marks mandatory, and lists every affected LCM Item. There is no second receiver-entry workflow: each nonserialized lot item receives one automatic assignment for its LCM receipt quantity, with Receipt Inventory Number `Shipment Number + Item`. A required Bin preserves the receipt-location default sourced by NetSuite, with `DEFAULTS.itemReceipt` as fallback. A required Inventory Status resolves the active standard NetSuite default status if the transformed receipt does not expose one, then falls back to `DEFAULTS.itemReceipt` only if that account lookup is unavailable. Serialized items and items requiring an Expiration Date are blocked because actual physical serial and supplier expiry values cannot be generated by LCM.
