...
- A line with **no** marker either predates the marker or was added by hand, and nothing in the data distinguishes those two. Appends therefore never touch an unmarked line - they add their own line instead. Only the repair flow can consolidate unmarked lines, and only after listing each one individually and taking a second explicit confirmation. That is the user attesting to what the lines are; the documentation does not present it as proof.
- A cost line exists to carry its Cost Category, so a line that cannot be tagged is a failure and not a partial success: untagged, it can never be picked up as a landed cost source, yet the Bill would save and the source rows would be marked `Created`, quietly delivering nothing. Creating a new line therefore aborts the whole run with a message naming the row, the category, the item, and the three NetSuite prerequisites; no Vendor Bill is saved and no row is marked `Created`.
...
- By default the repair consolidates only marker-matched lines, which needs no judgement from anyone. Repairing a Vendor Bill generated before the marker existed needs its unmarked lines included: the preview then lists every such line with its Bill, line number, category, item, amount, and description, and a separate confirmation button. Matching is re-done against the freshly loaded record at confirmation time rather than trusted from the preview, because the Bill can change in between.
- The repair refuses a Vendor Bill whose Landed Cost rows disagree about the Allocation Method, and reports why, for the same reason `Create Bill` refuses one: consolidating by category and item across rows that wanted different methods would merge lines that the single header-level method cannot describe. Historical Bills get the same rule as new ones rather than being partially repaired.
