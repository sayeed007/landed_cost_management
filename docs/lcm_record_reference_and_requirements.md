...
| Cost Allocated In GRN | `custrecord_lcm_lcm_cost_allocation_grn` | Checkbox | Indicates whether the created cost has been allocated to tracked item rows. Checked only after item-level allocation succeeds; created-but-unallocated Bill rows can be repaired with `Recalculate Landed Cost`. Duplicate cost lines on an already-created Vendor Bill are corrected with the separate `Repair Vendor Bill Lines` action, which only rewrites lines carrying this record's `custcol_lcm_source_key` marker unless unmarked lines are explicitly confirmed. |
