...
- Build every transaction in a run before saving any of them, so such a failure aborts with nothing saved even when the LCM record spans several vendors. Saving stays one record at a time, so report partial success explicitly - which transactions were written, and that re-running appends rather than duplicates - instead of claiming nothing was saved.
...
- No action rewrites an already-created Vendor Bill except appending compatible new rows to it. `Recalculate Landed Cost` repairs allocation only.
