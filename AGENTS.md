...
- An append must never touch an unmarked line. Only the repair flow may consolidate unmarked lines, and only after listing each one individually and taking a second explicit confirmation from the user. A lone unmarked line is still offered in that mode, so it can be adopted and stop attracting duplicates.
...
- `Cost Allocation Method` is a Vendor Bill body field, so one Bill carries one method. Refuse a vendor/subsidiary/currency group whose rows disagree about it rather than writing a Bill whose header misdescribes some of its cost lines, and apply the same refusal in `Repair Vendor Bill Lines` so historical Bills are not partially consolidated. Do not change the grouping rule to work around this.
