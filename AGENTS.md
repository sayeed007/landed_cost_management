...
- An append must never touch an unmarked line. Only the repair flow may consolidate unmarked lines, and only after listing each one individually and taking a second explicit confirmation from the user.
- A new cost line that cannot be tagged with its Cost Category must abort the run. Never save a Vendor Bill and mark rows `Created` around a line that can never act as a landed cost source.
